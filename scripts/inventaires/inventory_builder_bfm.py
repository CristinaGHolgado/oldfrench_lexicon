...
def prep_file(file):
	...
	df = pd.read_csv(file, encoding="utf-8", sep="\t", quoting=csv.QUOTE_NONE, header=None, index_col=False)
	df = df.iloc[:,:10]
	...
	df['form'] = df['form'].apply(lambda x: re.sub('\s|_', '-', x))
	...
	# remove numbers, asteriscs, lower from forms/lemmas/flp
	for col in df.columns:
		if col in ['form', 'lemma', 'flp']:
			df[col] = df[col].apply(lambda x: re.sub(r"\d+|\*","", x.lower()))
...
def make_inventory(folder_corpus, **ignore):
 	...
 	folder = [os.path.join(path, name) for path, subdirs, files in os.walk(folder_corpus) for name in files]

 	# Prepare & concat .conllu files
 	df = [prep_file(file) for file in folder if file not in ignore_files]
 	concated_df = pd.concat(df, axis=0)

 	print("-- Input files:", len([file for file in folder if file not in ignore_files]))
 	print("-- Number of forms (PUNCT ignored):", len(concated_df))
 	
 	# add col: frequence form-lemma-pos
 	concated_df['freq'] = 1

 	# Make invetory of forms
 	inv = concated_df.groupby(['flp','form','lemma']).agg({'freq': [lambda x: x.sum()],
 	...
 	print(f"-- POS distribution \n {concated_df.groupby('cattex')['cattex'].count().sort_values(ascending=False)}")
