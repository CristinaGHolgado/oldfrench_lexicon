...
parser.add_argument("-filter_pos", "--filter_pos", type=str, help="<str> Construct lexicon for specific POS", required=False)
...
# Detect file encoding & pass encoding to pandas csv
	check_encoding = open(file, 'rb').read()
	file_encoding = chardet.detect(check_encoding)['encoding'].lower()

	
	# read file, select first 9 cols, fill nans & add col names

	df = pd.read_csv(file, encoding=file_encoding, sep="\t", quoting=csv.QUOTE_NONE, names=list(range(20)))
	df = df.iloc[:,:9]
	df = df.fillna("_")
...
if  os.listdir(args.input_dir) == []:
	...
	df = [ofrlex_inventory(f) for f in folder if f not in list_ignore_files]
	inventory_ofrlex = pd.concat(df, axis=0)

	# save merged lexicon
	if args.filter_pos:
		inventory_ofrlex.to_csv(str(args.outputname + str(args.filter_pos).upper() + ".tsv"), sep='\t', encoding='utf-8', index=None, quoting=csv.QUOTE_NONE)
	else:
